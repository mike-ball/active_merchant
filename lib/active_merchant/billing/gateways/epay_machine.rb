module ActiveMerchant #:nodoc:
  module Billing #:nodoc:
    # This was copied from authorize_net.rb
    # The ePay Machine gateway can accept the Auth.Net 3.1 APIs, but uses a different URL
    class EPayMachine < Gateway
      API_VERSION = '3.1'

      class_attribute :test_url, :live_url, :arb_test_url, :arb_live_url

      self.test_url = "https://secure.epaymachine.com/gw/transact/anet.dll"
      self.live_url = "https://secure.epaymachine.com/gw/transact/anet.dll"

      self.arb_test_url = 'https://secure.epaymachine.com/gw/transact/anet.dll'
      self.arb_live_url = 'https://secure.epaymachine.com/gw/transact/anet.dll'

      APPROVED, DECLINED, ERROR, FRAUD_REVIEW = 1, 2, 3, 4

      RESPONSE_CODE, RESPONSE_REASON_CODE, RESPONSE_REASON_TEXT = 0, 2, 3
      AUTH_CODE, AVS_RESULT_CODE, TRANSACTION_ID, CARD_CODE_RESPONSE_CODE  = 4, 5, 6, 38

      self.supported_countries = ['US']
      self.supported_cardtypes = [:visa, :master, :american_express, :discover]
      self.homepage_url = 'https://secure.epaymachine.com/gw/login'
      self.display_name = 'ePay Machine (AuthNet)'
      self.abbreviation = 'ePay'
      ...
      def purchase(money, creditcard, options = {})
        ...
        add_address(post, options)
        add_customer_data(post, options)
        post.merge!({duplicate_window: 0}) # now you will never get the Duplicate Transaction response
      ...
      def credit(money, identification=nil, options = {})
        requires!(options, :card_number)

        post = { card_num: options[:card_number] }
        if identification.nil? #do an unlinked credit (identification cannot be submitted on unlinked credits.)
          year  = sprintf("%.4i", options[:expiration_year])
          month = sprintf("%.2i", options[:expiration_month])
          post.merge!({exp_date: "#{month}#{year[-2..-1]}" })
        else #do a linked credit
          post.merge!({trans_id: identification})
        end
      ...
      def commit(action, money, parameters)
        ...
        gateway_request = post_data(action, parameters)
        data = ssl_post url, gateway_request
        ...
        Response.new(success?(response), message, response,
          ...
          gateway_reason_code: response[:response_reason_code],
          standard_response: response[:response_code],
          transaction_id: response[:transaction_id]
        )
      end

      # remove apostrophes and other invalid characters
      def cleanse_data(parameters)
        parameters.each { |k, v| v.gsub!(/[']/, '') if v.class == String }
      end
...
#        end
        ...
        return results[:response_reason_text].nil? ? '' : results[:response_reason_text].strip
      end
